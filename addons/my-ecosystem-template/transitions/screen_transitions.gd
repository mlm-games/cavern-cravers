...
func change_scene_with_transition(scene_path: String, anim_name: String = "fadeToBlack", pop_up: bool = false, speed_scale: float = 1) -> void:
	transition(anim_name, false, speed_scale, pop_up)
	await screen_covered
	if !pop_up:
			A.tree.change_scene_to_file(scene_path)

func change_scene_with_transition_packed(scene: PackedScene, anim_name: String = "fadeToBlack", pop_up: bool = false, speed_scale: float = 1) -> void:
	transition(anim_name, false, speed_scale, pop_up)
	await screen_covered
...
func transition(anim_name: StringName = "fadeToBlack", single_transition_only: bool = false, speed_scale: float = 1, pop_up: bool = false) -> void:
	...
	match anim_name:
			...
			"circleIn":
					transition_player.speed_scale = speed_scale
					transition_rect.material.shader = C.RESOURCES.SHADERS.CIRCULAR_ENGULF
